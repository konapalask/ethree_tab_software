        ...
        <div className="group relative bg-white rounded-xl shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 border border-slate-100 overflow-hidden flex flex-col h-full">
            <div className="h-24 overflow-hidden relative">
                ...
                <div className="absolute bottom-1.5 left-2 right-2 flex items-end justify-between">
                    <h3 className="font-bold text-xs text-white leading-tight shadow-sm drop-shadow-md">{ride.name}</h3>
                    <span className="bg-amber-400 text-slate-900 text-[9px] font-bold px-1 py-0.5 rounded shadow-lg shadow-black/20">
                        ₹{ride.price}
                    </span>
                </div>
            </div>
            ...
            <div className="p-2 flex flex-col flex-1 bg-white relative">
                <p className="text-slate-500 text-[9px] mb-1.5 line-clamp-2 leading-relaxed">{ride.description}</p>

                <div className="mt-auto">
                    <button
                        onClick={() => onAdd(ride)}
                        className="w-full bg-slate-50 hover:bg-slate-900 text-slate-700 hover:text-amber-400 border border-slate-200 hover:border-slate-900 font-bold py-1 px-1.5 rounded-lg flex items-center justify-center gap-1 transition-all duration-300 group/btn text-[10px]"
                    >
                        <span className="bg-slate-200 group-hover/btn:bg-amber-400 p-0.5 rounded-full transition-colors">
                            <Plus size={10} className="text-slate-600 group-hover/btn:text-slate-900" />
                        </span>
                        Add
                    </button>
                </div>
